...
def request_file_from_peer(peer, filename, token):
...
def get_peer_list():
    try:
        ...
        for peer in peers:
            try:
                response = requests.get(f"http://{peer}/files", headers={"Authorization": AUTH_TOKEN}, timeout=2)
                if response.status_code == 200:
...
def upload_files():
    ...
    if file_path:
        ...
        headers = {'Authorization': token}
        try:
            response = requests.post(f"http://127.0.0.1:{PEER_PORT}/upload", files=files, headers=headers)
            files['file'].close()

            if response.status_code == 200:
                log_message(f"[SUCCESS] File '{file_name}' uploaded to your peer server")
            else:
                log_message(f"[ERROR] Upload failed: {response.json()}")
        except Exception as e:
            log_message(f"[ERROR] Upload error: {str(e)}")
...
def download_file_gui():
    ...
    for peer in peers:
        try:
            response = requests.get(f"http://{peer}/files", headers={"Authorization": token})
            if response.status_code == 200:
    ...
    try:
        ...
        request_file_from_peer(peer, filename.strip(), token)
    except Exception as e:
